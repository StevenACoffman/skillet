// Package judge is the deterministic rule-judge for behavioral scoring. It
// scores an output against a set of rule checks the same way SkillOpt-Sleep's
// judges.py does: hard = 1.0 iff every check passes, soft = passed/total. It is
// pure — values in, values out, no I/O, no model — so it is the first-line
// scoring mechanism, with a model reserved only for the remainder.
package judge
