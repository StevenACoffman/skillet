...
// targets are should_trigger prompts, distractors are should_not_trigger
// prompts, and "firing" means the description's vocabulary overlaps the prompt.
type Report struct {
...
// Score rates trigger accuracy for a description against its prompts. A target
// "fires" (TP) when it shares a salient term with the description, else misses
// (FN); a distractor firing is a false positive (FP), else a true negative (TN).
// NetUtility = (TP-FP)/total. TPR/FPR carry Wilson 95% intervals. Pure.
